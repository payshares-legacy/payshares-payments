...
* @param {object} network The paysharesd network.
*/
...
// A transaction has been included in our paysharesd's closed ledger, but not validated through consensus yet
Submitter.errors.PendingError                = Error.subclass("PendingError");
...
Submitter.prototype._handleSubmitError = function (err, transaction) {
    switch (err.name) {
        case "ApplyingTransaction":
            // paysharesd has already seen this transaction and it will be included shortly
            return;
        case "PastSequenceError":
            return this._confirmTransaction(transaction)
                .catch(Submitter.errors.PendingError, function () {
                    // this transaction has been applied but not yet validated, will be tesSUCCESS shortly
                    return;
                })
...
Submitter.prototype._isTransactionInLedger = function(hash) {
    return Promise.bind(this)
        ...
        .then(function (response) {
            ...
            // TODO: remove below after tesPENDING change deployed
            ...
            if (result === "tesPENDING") {
                throw new Submitter.errors.PendingError();
            }
