...
var UInt160 = require("payshares-lib").UInt160;
...
* @param {number|object} amount The amount of paysharess to send or the amount object to send (value/currency/issuer pair).
* @param {number} [amount.value] The amount of the currency to send.
* @param {string} [amount.currency] The currency to send (USD, EUR, etc). If unspecified, will default to XPR.
...
Client.prototype.createNewPayment = function (address, amount, memo) {
    if (!UInt160.is_valid(address)) {
        throw new Error("Address must be a valid Payshares address");
    }
...
function validateAmount(amount) {
    ...
    if (typeof(amount) === "object") {
        ...
        if (amount.issuer && !UInt160.is_valid(amount.issuer)) {
            throw new Error("Issuer must be a valid payshares address");
        }
    }
