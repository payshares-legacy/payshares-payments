...
Payments.prototype.checkFatalError = function () {
    ...
    return this.database.isAborted(this.fatalError.transaction)
        .then(function (result) {
            if (!result) {
                self.log.error("There's been a fatal error, aborting");
                return Promise.reject(self.fatalError);
            } else {
                var transaction = self.fatalError.transaction;
                self.fatalError = null;
                return self._handleResignError(transaction);
            }
        });
};
